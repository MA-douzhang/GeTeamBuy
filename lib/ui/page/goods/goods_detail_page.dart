...
import '../../../view_model/goods_detail_view_model.dart';
...
class _GoodsDetailPageState extends State<GoodsDetailPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return ChangeNotifierProvider<GoodsDetailViewModel>(
      ...
      child: Selector<GoodsDetailViewModel, GoodsDetailEntity>(
        ...
        selector: (BuildContext context, GoodsDetailViewModel model) {
          return model?.goodsDetailEntity ?? GoodsDetailEntity();
        },
      ),
    );
  }
                ...
                children: <Widget>[
                  ...
                  ChangeNotifierProvider.value(
                    ...
                    child: Selector<GoodsDetailViewModel, int>(builder:
                      ...
                      return model?.specificationId ?? 0;
                    }),
                  ),
                  Padding(
                    padding: EdgeInsets.only(
  ...
  _addCart() {
    SharedPreferencesUtil.getInstance()
      ...
      if (value != null) {
        _cartViewModel
            ?.addCart(_goodsDetailViewModel.goodsDetailEntity?.info?.id ?? 0,
                _goodsDetailViewModel?.specificationId ?? 0, _number)
            .then((response) {
          if (response) {
  ...
  _buy() {
    SharedPreferencesUtil.getInstance()
      ...
      if (value != null) {
        _goodsDetailViewModel
            .buy(_goodsDetailViewModel.goodsDetailEntity?.info?.id ?? 0,
                _goodsDetailViewModel?.specificationId ?? 0, _number)
            .then((response) {
          if (response > 0) {
            Navigator.of(context).pop(); //隐藏弹出框
      ...
      specificationsWidget.add(Container(
        child: ChoiceChip(
          label: Text(element?.name ?? '',
              style: specificationId == element.valueList?.first.id
          ...
          shape: RoundedRectangleBorder(
              ...
              side: BorderSide(
                color: specificationId == element.valueList?.first.id
                    ? AppColors.COLOR_FF5722
                    : AppColors.COLOR_999999,
          ...
          selected: specificationId == element.valueList?.first?.id,
          onSelected: (value) => {
            if (value)
              {
                _goodsDetailViewModel
                    .setSpecificationId(element.valueList?.first.id ?? 0)
              }
          },
        ),
      ));
