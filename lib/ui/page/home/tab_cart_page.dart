...
import '../../../ui/widgets/cart_number.dart';
...
//                      _cartViewModel _setCartItemCheck(bool);
  ...
  _setCheckAll(bool check) {
    List<int> ids = [];
    int count = _cartViewModel?.cartEntity?.cartList?.length ?? 0;
    for (int i = 0; i < count; i++) {
      if (_cartViewModel?.cartEntity?.cartList?[i].checked != check) {
        ids.add(_cartViewModel?.cartEntity?.cartList?[i].productId ?? 0);
      }
    ...
    if (cartViewModel.pageState == PageState.hasData) {
      return ListView.builder(
        itemBuilder: (context, index) {
          return _cartItemView(cartViewModel.cartEntity?.cartList?[index] ?? CartBean() , index);
        },
        itemCount: cartViewModel.cartEntity?.cartList?.length ?? 0,
      );
    }
            ...
            child: Row(
              ...
              children: <Widget>[
                Checkbox(
                    ...
                    onChanged: (bool) {
                      _cartViewModel?.checkCartItem(cartBean.productId ?? 0, bool ?? false);
                    }),
                CachedImageView(
                        ...
                        children: [
                          ...
                          Expanded(
                            ...
                            child: CartNumberView(cartBean.number, (value) {
                              print(value);
                              _cartViewModel?.updateCartItem(cartBean?.id ?? 0, value,
                                  cartBean.productId ?? 0, cartBean.goodsId ?? 0, index);
                            }),
                          ))
                        ],
                      )
